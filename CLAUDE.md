...
[2026-06-21] Added: supplement_schedule table driven Meds tab + Settings CRUD — loadSupplementSchedule/getTodayScheduledSupplements/renderMeds rewritten; settings panel SUPPLEMENTS & MEDS section with add/edit/delete form, 7-day checkboxes, active toggle; week tab lookup updated to use supplementSchedule state; SUPP_TIME_ORDER/SUPP_TIME_LABELS constants, ~lines 1092-1133 (HTML), 5086-5210 (JS)
[2026-06-21] Fixed: Settings panel not appearing — wrapped all render calls in openSettings() with try/catch so panel always opens; added Array.isArray guard in loadSupplementSchedule(); added background reload+re-render of supplement schedule on every settings open (fixes stale data if table created after boot)
[2026-06-21] Moved: Supplements & Meds from Settings panel to hamburger menu as top-level item — added #supplPanel (mirrors journalPanel), openSupplements()/closeSupplements() functions at ~line 5585; removed from openSettings(); hamburger menu now has Journal / Supplements & Meds / Settings nav rows
[2026-06-21] Added: Yesterday section in food search sheet — loadYesterdaySection() fetches prior day food_log using getLocalDateStr(new Date(Date.now()-86400000)), logs raw response, renders grouped by MEAL_SLOTS; reuseYdayEntry() reconstructs per_100g macros and opens food detail; clears and re-renders when search input cleared; ~lines 1498, 3736-3821
[2026-06-21] Added: video link button (📹) in scheduled exercise card header — renders only when ex.video_link non-empty; uses data-url + this.dataset.url pattern to avoid quote injection; stopPropagation prevents card toggle; .sched-video-btn CSS ~line 603, template ~line 2602
[2026-06-22] Fixed: cardio card fields (Incline+Distance missing), lbs label bleed, interval mode missing — root cause: 'treadmill' not in CARDIO_NAMES so exercises fell into isTimed+hasCustom path; added 'treadmill' to CARDIO_NAMES in renderScheduledWorkout (~line 2431) and schedExSaveOne (~line 2679); added Distance field to simple-mode UI and save logic; distance now saved in scheduledExercises JSONB
